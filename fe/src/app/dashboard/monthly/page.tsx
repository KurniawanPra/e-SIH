...
import { api, getCurrentUser, getApiError } from '@/lib/api'
import {
  CheckCircle2,
  Clock,
  AlertCircle,
...
export default function MonthlyActivitiesPage() {
  ...
  const fetchHighlights = async (month: number, year: number) => {
    ...
    try {
      ...
      if (version === requestVersion.current) setHighlights(r.data.data || [])
    } catch (err) {
      if (version === requestVersion.current) { setHighlights([]); setLoadError(getApiError(err, 'Highlight gagal dimuat')) }
    } finally {
              ...
              {filteredHighlights.length === 0 ? (
                ...
                paginatedHighlights.map((h, idx) => (
                  <div key={h.id} className="p-3.5 rounded-2xl bg-white border border-slate-200 shadow-2xs space-y-2.5">
                    ...
                    {/* Title & Description */}
                    <div>
                      <button type="button" onClick={() => setDetailActivity(h)} className="font-bold text-brand-800 text-sm leading-snug text-left hover:underline">{h.item}</button>
                      {h.description && (
                    ...
                    paginatedHighlights.map((h, idx) => (
                      <tr key={h.id} className="border-b border-slate-100 align-top hover:bg-slate-50">
                        ...
                        <td className="px-2.5 py-3 font-bold text-slate-800 whitespace-pre-wrap">
                          ...
                          <button type="button" onClick={() => setDetailActivity(h)} className="text-left hover:underline text-brand-800">{h.item}</button>
                        </td>
