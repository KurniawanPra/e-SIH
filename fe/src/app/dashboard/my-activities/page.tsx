...
import { hasActivityPic, activityDate } from '@/lib/activities'
...
export default function MyActivitiesPage() {
  ...
  const fetchAll = async () => {
      ...
      const [u, r1, r2, r3] = await Promise.allSettled([
        getCurrentUser(),
      ...
      setActivities(r1.status === 'fulfilled' ? r1.value.data.data || [] : [])
      setItemPrograms(r2.status === 'fulfilled' ? r2.value.data.data || [] : [])
      setParentPrograms(r3.status === 'fulfilled' ? r3.value.data.data || [] : [])
      setLoadError([u, r1, r2, r3].some(result => result.status === 'rejected') ? 'Sebagian data gagal dimuat. Silakan coba lagi.' : '')
      setLoading(false)
  }
  ...
  const myActivities = useMemo(() => {
    if (!user) return []
    return activities.filter((a: any) => hasActivityPic(a, user))
  }, [activities, user])

  // Filtered by year, status, and search query
  const filteredActivities = useMemo(() => {
    return myActivities.filter((a: any) => {
      const year = Number(activityDate(a)?.slice(0, 4))
  ...
  return (
    <div className="space-y-6 mb-12 sm:mb-16">
      {loadError && <div role="alert" className="p-4 rounded-xl bg-amber-50 text-amber-900 text-sm">{loadError} <button type="button" onClick={fetchAll} className="underline font-semibold">Coba lagi</button></div>}
      {/* Header Row */}
