...
import { getCurrentUser, api, getApiError } from '@/lib/api'
import {
  Plus,
  Pencil,
  X,
...
import { activityDate, activityWeek, hasActivityPic, inDateRange, localToday, periodRange, type ActivityWeek } from '@/lib/activities'
...
const weekLabel = (startDate?: string, dueDate?: string) => activityWeek(startDate || dueDate)
...
export default function WeeklyActivitiesPage() {
  ...
  const [selectedWeek, setSelectedWeek] = useState<ActivityWeek | 'CUSTOM'>('ALL')
  ...
  // Preset controls and date inputs share one inclusive range, based on start date.
  const initialRange = periodRange(selectedYear, currentMonthIdx)
  const [startDateFilter, setStartDateFilter] = useState(initialRange.start)
  const [endDateFilter, setEndDateFilter] = useState(initialRange.end)
  const applyPeriod = (month: number, year: number, week: ActivityWeek = 'ALL') => {
    const range = periodRange(year, month, week)
  ...
  useEffect(() => {
    ...
    applyPeriod(selectedMonth, selectedYear, selectedWeek === 'CUSTOM' ? 'ALL' : selectedWeek)
  }, [selectedYear, selectedMonth, selectedWeek])
  const resetPeriod = () => applyPeriod(currentMonthIdx, selectedYear)
  ...
  const fetchAll = async () => {
    ...
    setLoadError('')
    const [activityResult, directoryResult] = await Promise.allSettled([
      api.get('/api/esih/activities'),
      api.get('/api/esih/users'),
    ])
    if (version !== fetchVersion.current) return
    if (activityResult.status === 'fulfilled') { setActivities(activityResult.value.data.data || []); setLoadError('') }
    else {
      setActivities([])
      setLoadError(getApiError(activityResult.reason, 'Laporan aktivitas gagal dimuat. Silakan coba lagi.'))
    }
    if (directoryResult.status === 'fulfilled') setUsersList(directoryResult.value.data.data || [])
    setLoading(false)
  }
  ...
  const filteredActivities = useMemo(() => {
    const list = activities.filter((a: any) => {
      if (a.isActive === false || !inDateRange(activityDate(a), startDateFilter, endDateFilter)) return false
      if (user?.role !== 'ADMIN' && !hasActivityPic(a, { name: user?.name, email: user?.email })) return false

      // User Filter
      if (userFilter !== 'ALL') {
        if (!hasActivityPic(a, { name: userFilter })) return false
      }
    ...
    return list.sort((a: any, b: any) => {
      const scoreA = pinnedIds.includes(a.id) ? 1000 : 0
      const scoreB = pinnedIds.includes(b.id) ? 1000 : 0
      if (scoreA !== scoreB) {
        return scoreB - scoreA
      }
      return new Date(b.startDate || 0).getTime() - new Date(a.startDate || 0).getTime()
    })
  }, [activities, startDateFilter, endDateFilter, userFilter, statusFilter, search, user, pinnedIds])
  ...
  const submitForm = async (e: React.FormEvent) => {
    ...
    try {
      if (editItem) {
        ...
        toast.success('Laporan aktivitas mingguan berhasil diperbarui', 'Sukses Diperbarui')
      } else {
        await api.post('/api/esih/activities', form)
        toast.success('Laporan aktivitas mingguan baru berhasil ditambahkan', 'Sukses Ditambahkan')
      }
      setShowModal(false)
      fetchAll()
    } catch (error) {
      toast.error(getApiError(error, 'Gagal menyimpan data aktivitas'), 'Terjadi Kesalahan')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const submitQuickStatus = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Status aktivitas berhasil diubah menjadi ${statusForm.status}`, 'Status Diperbarui')
      setShowStatusModal(false)
      fetchAll()
    } catch (error) {
      toast.error(getApiError(error, 'Gagal memperbarui status aktivitas'), 'Terjadi Kesalahan')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div className="space-y-5 pb-16 sm:pb-24">
      ...
      {loadError && <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">{loadError} <button onClick={fetchAll} className="ml-2 underline font-semibold">Coba lagi</button></div>}
      {startDateFilter > endDateFilter && endDateFilter && <p role="status" className="text-sm text-amber-800">Tidak ada tanggal pada periode ini. Pilih rentang awal yang tidak melewati tanggal akhir.</p>}
      {/* Header Row */}
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
        ...
        <p className="text-xs text-slate-600 font-medium mt-1">
          Laporan pekerjaan yang dilakukan user dalam 1 minggu - dilacak dari Tanggal Mulai, plus waktu upload laporan
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-white p-3.5 rounded-xl border border-slate-200 shadow-xs">
        {/* Left: Filter & Export Buttons */}
        <div className="flex flex-wrap items-center gap-2.5">
          <button
            onClick={() => setShowFilters(!showFilters)}
            className={`inline-flex items-center justify-center gap-2 font-semibold text-xs px-3.5 py-2 rounded-lg cursor-pointer transition-colors ${
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 font-semibold text-xs px-3.5 py-2 cursor-pointer shadow-2xs transition-colors"
        ...
        <div>
          <button
            onClick={openAdd}
            className="inline-flex w-full sm:w-auto items-center justify-center gap-2 bg-brand-700 hover:bg-brand-800 text-white font-semibold text-xs px-4 py-2 rounded-lg cursor-pointer transition-colors shadow-xs"
          >
            <Plus size={15} />
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-3.5">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 flex-wrap">
            ...
            <div className="flex flex-wrap items-center gap-1.5 bg-slate-50 p-1.5 rounded-xl border border-slate-300">
              ...
              <input
                ...
                onChange={e => { setSelectedWeek('CUSTOM'); setStartDateFilter(e.target.value) }}
                onInput={e => { setSelectedWeek('CUSTOM'); setStartDateFilter(e.currentTarget.value) }}
              ...
              <input
                type="date"
                value={endDateFilter}
                onChange={e => { setSelectedWeek('CUSTOM'); setEndDateFilter(e.target.value) }}
                onInput={e => { setSelectedWeek('CUSTOM'); setEndDateFilter(e.currentTarget.value) }}
                className="px-2 py-1 rounded-lg bg-white border border-slate-300 text-xs font-bold text-slate-900 outline-none focus:border-brand-500"
                title="Tanggal Akhir"
              />
        ...
        <div className="pt-2.5 border-t border-slate-200">
          <span className="block text-xs font-bold text-slate-700 mb-2">Pilih Minggu (berdasarkan Tanggal Mulai):</span>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-1.5">
            {[
              { id: 'ALL', label: 'Semua Minggu' },
              { id: 'W1', label: 'M1 (Tgl 1–7)' },
              { id: 'W2', label: 'M2 (Tgl 8–14)' },
              { id: 'W3', label: 'M3 (Tgl 15–21)' },
              { id: 'W4', label: 'M4 (Tgl 22–28)' },
              { id: 'W5', label: 'M5 (Tgl 29–31)' },
            ].map(week => {
              const active = selectedWeek === week.id
              return (
                <button
                  key={week.id}
                  onClick={() => applyPeriod(selectedMonth, selectedYear, week.id as ActivityWeek)}
                  aria-pressed={active}
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0">
                  <button type="button" onClick={() => setDetailActivity(a)} className="text-left font-semibold text-brand-800 text-sm leading-snug hover:underline break-words">{a.kegiatan}</button>
                  ...
                  return (
                    ...
                    <td className="py-4 px-4">
                      <div className="space-y-1">
                        ...
                        <button type="button" onClick={() => setDetailActivity(a)} className="text-left font-semibold text-brand-800 text-xs leading-snug hover:underline break-words">{a.kegiatan}</button>
                        {a.descriptionAction && (
                          <button type="button" onClick={() => setDetailActivity(a)} aria-label={`Baca detail ${a.kegiatan}`} className="block text-left text-xs text-slate-600 font-medium line-clamp-2 hover:text-brand-700">{a.descriptionAction}</button>
                        )}
                      </div>
                    </td>
