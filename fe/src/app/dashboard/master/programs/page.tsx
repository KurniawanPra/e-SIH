...
export default function DaftarProgramKerjaPage() {
  ...
  const assignedProgramIds = useMemo(() => {
    if (isAdmin) return []
    return currentUser?.programIds || ((userObj as any)?.programs || []).map((p: any) => String(p.programId || '')).filter(Boolean)
  }, [isAdmin, userObj, currentUser])

  // Flag: non-admin user with no program kerja assigned by admin
  const hasNoAssignment = !isAdmin && currentUser !== null && assignedProgramIds.length === 0
  ...
  const displayParents = useMemo(() => {
    ...
    const filtered = parents.filter(p => {
      ...
      return assignedProgramIds.some((id: string) => {
        ...
        return upper === pId || upper === pKode || upper === `PK-${pKode}` || upper.startsWith(`${pKode}.`) || p.items.some(sub => sub.id.toUpperCase() === upper)
      })
    })
  ...
  const currentParent = useMemo(() => {
    return displayParents.find(p => p.id === activeParentTab) || displayParents[0] || null
  }, [displayParents, activeParentTab])
  ...
  const currentSubItems = useMemo(() => {
    ...
    if (isAdmin || assignedProgramIds.some((id: string) => [currentParent?.id, currentParent?.kode, `PK-${currentParent?.kode}`].includes(id))) return items
    return items.filter(item => assignedProgramIds.includes(item.id))
  }, [currentParent, isAdmin, assignedProgramIds])
                          ...
                          return (
                            <tr key={act.id} className="hover:bg-slate-50/80 transition-colors align-top">
                              ...
                              <td className="py-3.5 px-3.5 font-semibold text-slate-900 leading-snug">
                                <button type="button" onClick={() => openAuditModal(act)} className="text-left text-brand-800 hover:underline">{act.kegiatan}</button>
                              </td>
