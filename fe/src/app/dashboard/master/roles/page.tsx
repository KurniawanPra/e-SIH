...
interface MasterProgram { id: string; kode: string; namaProgram: string; deskripsi?: string; items?: Array<{ id: string }> }

function normalizeProgramIds(rawList: any[], masters: MasterProgram[]): string[] {
  const result = new Set<string>()
  for (const item of rawList) {
    const value = String(typeof item === 'string' ? item : item?.programId || '')
    const program = masters.find(p => p.id === value || p.kode === value || `PK-${p.kode}` === value)
    // Preserve subprogram IDs, including IDs from other years; never silently drop assignments.
    result.add(program?.id || value)
  }
...
export default function RolesManagementPage() {
  ...
  const [masterPrograms, setMasterPrograms] = useState<MasterProgram[]>([])
  ...
  const fetchUsers = async () => {
    try {
      const [res, programs] = await Promise.all([api.get('/api/esih/users'), api.get('/api/esih/program-kerja')])
      const masters: MasterProgram[] = programs.data.data || []
      setMasterPrograms(masters)
      setLoadError('')
      const data = res.data.data || []
      setUsers(data)
      const initial: Record<string, string[]> = {}
      data.forEach((u: any) => {
        initial[u.id] = normalizeProgramIds(u.programs || [], masters)
      })
      setDrafts(initial)
    } catch (e) {
      setLoadError(getApiError(e, 'Data penugasan gagal dimuat'))
    } finally {
      setLoading(false)
    }
  }
  ...
  const isDirty = (u: any) => {
    const current = normalizeProgramIds(u.programs || [], masterPrograms)
  ...
  const saveAssignment = async (u: any) => {
    ...
    try {
      ...
      console.error(err)
      toast.error(getApiError(err, 'Gagal menyimpan penugasan program kerja'), 'Terjadi Kesalahan')
    } finally {
      setSavingId(null)
    }
  }
  ...
  const getDisplayEmail = (u: any) => u.email || 'Email belum tersedia di Portal'

  return (
    <div className="space-y-6 pb-24 sm:pb-32">
      {loadError && <div role="alert" className="p-4 rounded-xl bg-red-50 text-red-800">{loadError} <button onClick={fetchUsers} className="underline">Coba lagi</button></div>}
      {/* Header */}
