...
import { Plus, Pencil, Eye, EyeOff, X, FolderKanban, ListChecks } from 'lucide-react'
...
export default function MasterParentPKPage() {
  ...
  const fetchParents = () => {
    api.get('/api/esih/program-kerja')
      .then(res => { setParents(res.data.data || []); setLoading(false) })
  ...
  const openEdit = (item: ParentPK) => { setEditItem(item); setForm({ kode: item.kode, namaProgram: item.namaProgram, deskripsi: item.deskripsi || '' }); setShowModal(true) }
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    } catch { alert('Gagal menyimpan data Program Kerja Induk') } finally { setSubmitting(false) }
  }

  const toggle = async (id: string) => {
    if (confirm('Apakah Anda yakin ingin mengubah status aktif program ini?')) {
      try { await api.patch(`/api/esih/program-kerja/${id}/toggle`); fetchParents() }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h2 className="text-lg sm:text-xl font-bold text-slate-900 flex items-center gap-2">
            <FolderKanban size={20} className="text-brand-700" /> Master Program Kerja (Induk)
          </h2>
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">Kelola kelompok utama Program Kerja dan pantau Total Progress Sub-Program.</p>
        </div>
        <button onClick={openAdd} className="w-full sm:w-auto flex items-center justify-center gap-2 neu-btn-brand font-extrabold text-xs px-4 py-2.5 rounded-xl cursor-pointer"><Plus size={16} /> Tambah PK Induk</button>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:hidden">
        {parents.length === 0 ? (
          <div className="bg-white p-8 rounded-2xl border-2 border-slate-300 text-center text-slate-400 font-semibold text-xs">
            Belum ada data Program Kerja Induk.
          </div>
        ) : (
          parents.map(p => (
            <div key={p.id} className={`bg-white rounded-2xl border border-slate-100 p-4 shadow-xs space-y-3 ${!p.isActive ? 'opacity-40' : ''}`}>
              ...
              <div className="flex items-center gap-2 text-[11px] text-slate-500 font-semibold">
                <ListChecks size={13} className="text-slate-400" /> {p.items?.length || 0} Item
              </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="bg-slate-50 border-b border-slate-100">
              ...
              <th className="text-left px-5 py-3 font-semibold text-slate-500 text-xs uppercase">Nama Program Kerja Induk</th>
              <th className="text-left px-5 py-3 font-semibold text-slate-500 text-xs uppercase">Deskripsi</th>
              <th className="text-left px-5 py-3 font-semibold text-slate-500 text-xs uppercase">Item</th>
              <th className="text-left px-5 py-3 font-semibold text-slate-500 text-xs uppercase w-48">Total Progress</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {parents.length === 0 ? (
                <tr><td colSpan={7} className="text-center py-10 text-slate-400 font-semibold text-sm">Belum ada data Program Kerja Induk.</td></tr>
              ) : (
                parents.map(p => (
                  <tr key={p.id} className={`hover:bg-slate-50 transition-colors ${!p.isActive ? 'opacity-40' : ''}`}>
                    ...
                    <td className="px-5 py-3.5"><p className="font-semibold text-slate-800">{p.namaProgram}</p></td>
                    <td className="px-5 py-3.5"><p className="text-xs text-slate-400 truncate max-w-xs">{p.deskripsi || '-'}</p></td>
                    <td className="px-5 py-3.5"><span className="inline-flex items-center px-2 py-0.5 rounded-md bg-slate-100 text-xs font-semibold text-slate-600">{p.items?.length || 0} Item</span></td>
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <div className="inline-flex gap-1">
                        <button onClick={() => openEdit(p)} className="p-1.5 rounded-md hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors cursor-pointer"><Pencil size={15} /></button>
                        <button onClick={() => toggle(p.id)} className={`p-1.5 rounded-md transition-colors cursor-pointer ${p.isActive ? 'hover:bg-amber-50 text-slate-400 hover:text-amber-600' : 'hover:bg-green-50 text-slate-400 hover:text-green-600'}`}>{p.isActive ? <EyeOff size={15} /> : <Eye size={15} />}</button>
                      </div>
                    </td>
                  </tr>
                ))
