        ...
        <div className="modal-backdrop fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-3 sm:p-4">
          <div className="modal-content bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            ...
            <form onSubmit={submit} className="p-4 sm:p-6 space-y-3.5 sm:space-y-4">
              <div><label className="block text-xs font-bold text-slate-700 mb-1">Item Program</label><select value={form.idProgram} onChange={e => setForm({ ...form, idProgram: e.target.value })} required className="w-full px-3 py-2.5 rounded-xl neu-select text-xs sm:text-sm font-bold text-slate-900 outline-none">{subOpts.map(s => <option key={s.id} value={s.id}>[{s.programKerja?.kode}] {s.kode} - {s.namaItem}</option>)}</select></div>
              <div><label className="block text-xs font-bold text-slate-700 mb-1">Action / Kegiatan</label><textarea rows={3} value={form.kegiatan} onChange={e => setForm({ ...form, kegiatan: e.target.value })} required className="w-full px-3 py-2.5 rounded-xl neu-input text-xs sm:text-sm font-bold text-slate-900 outline-none resize-none" /></div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div><label className="block text-xs font-bold text-slate-700 mb-1">Due Date</label><input type="date" value={form.dueDate} onChange={e => setForm({ ...form, dueDate: e.target.value })} required className="w-full px-3 py-2.5 rounded-xl neu-input text-xs sm:text-sm font-bold text-slate-900 outline-none" /></div>
                <div><label className="block text-xs font-bold text-slate-700 mb-1">PIC</label><input type="text" value={form.picNama} onChange={e => setForm({ ...form, picNama: e.target.value })} required className="w-full px-3 py-2.5 rounded-xl neu-input text-xs sm:text-sm font-bold text-slate-900 outline-none" /></div>
              </div>
              <div className="flex justify-end gap-2 pt-2"><button type="button" onClick={() => setShowModal(false)} className="px-4 py-2 rounded-xl text-xs sm:text-sm font-bold text-slate-600 neu-btn cursor-pointer">Batal</button><button type="submit" disabled={submitting} className="px-5 py-2 rounded-xl text-xs sm:text-sm font-extrabold bg-brand-700 text-white hover:bg-brand-800 disabled:opacity-50 cursor-pointer shadow-xs">{submitting ? 'Menyimpan...' : 'Simpan'}</button></div>
            </form>
          </div>
        </div>
      )}
