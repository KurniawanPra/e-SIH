...
## Corpus Check
- 318 files · ~291,388 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1657 nodes · 1646 edges · 155 communities (137 shown, 18 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 3 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a00cc50b`
...
10. `Prisma Database Setup` - 12 edges

## Surprising Connections (you probably didn't know these)
- `DashboardLayout()` --calls--> `getCurrentUser()`  [EXTRACTED]
  frontend/src/app/dashboard/layout.tsx → frontend/src/lib/api.ts
- `HeaderProps` --references--> `SessionUser`  [EXTRACTED]
  frontend/src/components/Header.tsx → frontend/src/types/auth.ts
- `buildApp()` --indirect_call--> `authRoutes()`  [INFERRED]
  backend/src/app.ts → backend/src/routes/auth.route.ts
- `buildApp()` --indirect_call--> `esihRoutes()`  [INFERRED]
  backend/src/app.ts → backend/src/routes/esih.route.ts
- `buildApp()` --indirect_call--> `portalDataRoutes()`  [INFERRED]
  backend/src/app.ts → backend/src/routes/portal-data.route.ts
...
### Community 2 - "Model Queries"
Cohesion: 0.22
Nodes (8): Covered vs Skipped, Health Score, ISSUE-001 — 401 di konsol saat halaman login (Severity: Low, Category: Console), ISSUE-002 — Tombol aksi tabel desktop bertuliskan "Update" tanpa kata "Status" (Severity: Low, Category: UX), ISSUE-003 — Tombol ikon tanpa aria-label (Severity: Low, Category: Accessibility), Issues, QA Report — e-SIH Operation, Ringkasan

### Community 4 - "chart.tsx"
Cohesion: 0.20
...
Cohesion: 0.27
Nodes (7): DashboardLayout(), BADGE_COLORS, MarqueeFooter(), nav, Sidebar(), SidebarProps, SessionUser
...
### Community 9 - "api.ts"
Cohesion: 0.15
Nodes (10): BACKEND_DRIVER, BackendDriver, logoutSession(), prepareCsrf(), ApiEnvelope, EmployeeGrade, OrganizationUnit, PlacementArea (+2 more)
...
Cohesion: 0.22
Nodes (10): ParentPK, ProgramKerjaPage(), SubItem, Header(), HeaderProps, useYear(), YearContext, YearContextType (+2 more)
...
### Community 56 - "api.ts"
Cohesion: 0.19
Nodes (10): MONTH_NAMES, MyActivitiesPage(), DashboardPage(), MONTH_NAMES, WeeklyActivitiesPage(), DemoLoginPage(), ModalPortal(), ModalPortalProps (+2 more)
...
## Knowledge Gaps
- **1130 isolated node(s):** `$schema`, `.opencode/plugins/graphify.js`, `@opencode-ai/plugin`, `docker-entrypoint.sh script`, `name` (+1125 more)
...
- **Why does `useChart()` connect `react` to `chart.tsx`?**
  _High betweenness centrality (0.004) - this node is a cross-community bridge._
